...
- The PM sees that there are already agents that are good at getting data from each of GitHub, Jira, and Slack, so adds those to the main agent, and then in plain english says:
...
- There's also a tool for searching previous messages, so they add that option, and continue descrbing what they want:
...
- There's already a report generating agent, so they add that too and say:
