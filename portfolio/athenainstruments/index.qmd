...
subtitle: "Full web application production ready"
---
...
When I was first using it, there was no customization available, only a chat interface. Once I figured out the some tactics for getting better responses (what has become "prompt engineering") I found it frustrating to keep having to repeat myself, I wanted to be able to set up templates for my workflows. But then, why stop there? There are so many other things that can be done behind the scenes to improve the productivity for the user. The technology was so new that no-one was offering a convenient interface for *any* customization, let alone **good** customization. This is where the idea for AthenaInstruments was born. Why not make it myself?
